"""Compare measured-forgetting replay with token-matched random replay."""
import argparse
import copy
import gc
import hashlib
...
PROBE=('Record the beacon hue registered for {name}.','For the site named {name}, return its beacon color.')
assert not set(PROBE)&set(TRAIN+EVAL+TRANSFER)
...
def main():
    ...
    def load(adapter,trainable):
        ...
        return result
    set_seed(args.seed)
    model=load(prior_run/'small_wave1_adapter',False)
    setup_seconds=time.perf_counter()-started
    width=max(len(ids) for ids,_ in examples(payload['candidates']+payload['latest']))
    ...
    def probe():
        model.eval();result={};tokens=0;torch.cuda.synchronize();started=time.perf_counter()
        with torch.inference_mode():
            for claim in payload['candidates']:
                values=[]
                for template in PROBE:
                    seq=prompt(template.format(name=claim['name']));tokens+=len(seq)
                    ids=torch.tensor([seq],device='cuda')
                    scores=model(input_ids=ids).logits[0,-1].float()[[x[0] for x in options]]
                    probabilities=scores.softmax(0).cpu().tolist()
                    values.append(probabilities[COLORS.index(claim['color'])])
                result[claim['name']]=values
        torch.cuda.synchronize()
        return dict(probabilities=result,seconds=time.perf_counter()-started,input_tokens=tokens,forward_passes=32)
    ...
    old_waves=json.loads((prior_run/'report.json').read_text())['arms']['small']['waves']
    before_eval=evaluate(eval_world=payload['before_world'])
    report['before_reload_delta']=max(abs(a-b) for x,y in zip(old_waves[0]['evaluation']['rows'],before_eval['rows']) for a,b in zip(x['probabilities'],y['probabilities']))
    assert report['before_reload_delta']<=1e-5
    report['before_evaluation']=before_eval
    before_probe=probe()
    del model;gc.collect();torch.cuda.empty_cache()
    started=time.perf_counter();model=load(prior_run/'small_wave2_adapter',True)
    report['affected_load_seconds']=time.perf_counter()-started
    prior={k:v.detach().cpu().clone() for k,v in get_peft_model_state_dict(model).items()}
    after_probe=probe()
    started=time.perf_counter()
    ranking=rank_losses(before_probe['probabilities'],after_probe['probabilities'])
    selected=sorted([c for c in payload['candidates'] if c['name'] in ranking['selected_names']],key=lambda c:c['name'])
    cost=lambda c:tuple(len(ids) for ids,_ in examples([c]))
    random_claims=matched_random(payload['candidates'],selected,cost,args.seed)
    report['selection_seconds']=time.perf_counter()-started
    report['probe_seconds']=before_probe['seconds']+after_probe['seconds']
    save(out/'selection.json',dict(before=before_probe,after=after_probe,ranking=ranking,selected=selected,random=random_claims))
    arms=dict(drift=payload['latest']+selected,random=payload['latest']+random_claims)
    assert len(arms['drift'])==len(arms['random'])==8
    save(out/'training_claims.json',arms)
    baseline=evaluate()
    old=old_waves[-1]['evaluation']['rows']
    ...
    report['prior_transfer_reload_delta']=max(abs(a-b) for x,y in zip(old_waves[-1]['transfer']['rows'],report['baseline_transfer']['rows']) for a,b in zip(x['probabilities'],y['probabilities']))
    ...
    model=load(out/'drift_adapter',False)
    fresh=evaluate();original=report['arms']['drift']['evaluation']['rows']
    ...
    report['transfer_reload_delta']=max(abs(a-b) for x,y in zip(report['arms']['drift']['transfer']['rows'],fresh_transfer['rows']) for a,b in zip(x['probabilities'],y['probabilities']))
