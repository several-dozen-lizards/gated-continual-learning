# Qwen3.5-2B: controlled provenance-gating pilot

## Scope and interpretation

Qwen/Qwen3.5-2B, revision recorded in model_snapshot.json, text-only causal loader,
4-bit NF4 with double quantization, BF16 computation; rank-16 adapters on all text
linear modules. Use the model's chat template with thinking disabled. These are
architecture/prompting changes from the parent 1.5B experiment, not a controlled
parameter-count-only comparison. No vision encoder is loaded.
