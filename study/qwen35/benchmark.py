"""Qwen3.5-2B: separate development learning check and paired synthetic-world trials."""
import argparse
import copy
import gc
import json
...
def main():
    ...
    def new_base():
        return Qwen3_5ForCausalLM.from_pretrained(snapshot['path'],local_files_only=True,
    ...
    model = get_peft_model(model,LoraConfig(r=16,lora_alpha=32,target_modules='all-linear',lora_dropout=0,bias='none',task_type='CAUSAL_LM'))
        ...
        with torch.inference_mode():
            for name,answer in targets.items():
                for template in EVAL_TEMPLATES:
                    ...
                    ids=torch.tensor([prompt_tokens(prompt)],device='cuda')
