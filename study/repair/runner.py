"""Repair an already learned false claim under a frozen continuation protocol."""
import argparse
import copy
import gc
import hashlib
...
from policy import COLORS,repair
...
def main():
    ...
    def load(adapter,trainable):
        ...
        return result
    set_seed(args.seed)
    model=load(prior_run/'reassess_replay_adapter',True)
    setup_seconds=time.perf_counter()-started
    prior={k:v.detach().cpu().clone() for k,v in get_peft_model_state_dict(model).items()}
    started=time.perf_counter();dynamic=repair(payload);routing_seconds=time.perf_counter()-started
    sham=repair(payload,sham=True)
    arms=dict(correction_only=dynamic['selected'],corrected_replay=dynamic['ledger'],unchanged_replay=sham['ledger'])
    assert len(arms['correction_only'])==1
    width=max(len(ids) for events in arms.values() for ids,_ in examples(events))
    save(out/'decisions.json',dict(dynamic=dynamic,sham=sham))
        ...
        with torch.inference_mode():
            for name,answer in world['final'].items():
                for template in templates:
                    ...
                    rows.append(dict(name=name,template=template,group=world['groups'][name],answer=answer,prediction=prediction,
    ...
    report=dict(status='running',seed=args.seed,setup_seconds=setup_seconds,routing_seconds=routing_seconds,arms={},
        versions={'torch':torch.__version__,'transformers':transformers.__version__,'peft':peft.__version__})
    baseline=evaluate()
    old=json.loads((prior_run/'report.json').read_text())['arms']['reassess_replay']['evaluation']['rows']
    ...
    for a,b in [('corrected_replay','unchanged_replay')]:
    ...
    model=load(out/'corrected_replay_adapter',False)
    fresh=evaluate();original=report['arms']['corrected_replay']['evaluation']['rows']
    ...
    report['transfer_reload_delta']=max(abs(a-b) for x,y in zip(report['arms']['corrected_replay']['transfer']['rows'],fresh_transfer['rows']) for a,b in zip(x['probabilities'],y['probabilities']))
